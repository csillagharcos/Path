...
from report_forms.c31.forms import C31Form, FileUploadForm, TrendForm, AnonymStatForm
...
def Statistics(request):
    ...
    if context:
        ...
        return render(request, 'c31_statistics.html', {"not_enough": True})
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            for workplace in workplaces:
                stat = CountStatistics(c31.objects.filter(added_by__personel__workplace = workplace, date_of_admission__gte = start, date_of_admission__lte = end ), False )
                if stat['counted'] >= 30:
                    statistics += [{
...
def SortAndAddCountryAverage(statistics, start, end, user):
    ...
    for country in hospitals:
        ...
        for hospit in country['hospitals']:
            query = c31.objects.filter(added_by__personel__workplace__codename = hospit, date_of_admission__gte = start, date_of_admission__lte = end )
