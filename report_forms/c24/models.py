...
class c24(models.Model):
    ...
    principal_diagnoses_code        = models.CharField(_('Principal diagnosis code (ICD-10 or DRG)'), max_length=5, default='')
    ...
    name_of_first_dose              = models.ForeignKey(Medicine, verbose_name=_('Generic name first used antibiotic drug'), related_name="First Dose", null=True, blank=True)
    first_dose                      = models.FloatField(_('First dose of first antibiotic drug'), max_length=10, null=True, blank=True)
    name_of_second_dose             = models.ForeignKey(Medicine, verbose_name=_('Generic name second used antibiotic drug'), related_name="Second Dose", null=True, blank=True)
    second_dose                     = models.FloatField(_('First dose of second antibiotic drug'), max_length=10, null=True, blank=True)
    name_of_other_dose              = models.CharField(_("Name of antibiotic drug"), max_length=255, null=True, blank=True)
    other_dose                      = models.FloatField(_('First dose of other antibiotic drug'), max_length=10, null=True, blank=True)
    ...
    class Meta:
        verbose_name = _('Planned surgery for hysterectomy')
        verbose_name_plural = _('Planned surgery for hysterectomy')

class c24CSV(CsvModel):
    case_id                         = CharField()
